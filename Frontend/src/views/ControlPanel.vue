<script setup lang="ts">
      ...
      const logContent = ref(""); // Store the log content
      ...
      const buttons = [
        ...
        { label: "Load Error Log", action: () => fetchLogContent() }, // New button for loading the log
      ];
      ...
      const fetchLogContent = async () => {
        try {
          const response = await fetch('http://localhost:8080/api/logs/WorkerSemaphore2.log');
          if (response.ok) {
          ...
          logContent.value = "An error occurred while fetching the log content.";
          console.error(error);
        }
      };
