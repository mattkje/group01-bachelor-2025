...
public class SimulationControllerTest {
  ...
  void testMonteCarlo() throws Exception {
    when(worldSimulationController.getCurrentDateTime()).thenReturn(
        ResponseEntity.ok(LocalDateTime.now()));
    when(worldSimulationController.getModels()).thenReturn(
        Map.of("model1", mock(RandomForest.class)));
    when(simulationService.runCompleteSimulation(any(), any())).thenReturn(
        Map.of(1L, List.of("Result1")));
  ...
  void testMonteCarloZone() throws Exception {
    ...
    when(worldSimulationController.getModels()).thenReturn(
        Map.of("model1", mock(RandomForest.class)));
    when(simulationService.runZoneSimulation(anyLong(), any(), any())).thenReturn(
        List.of(new ZoneSimResult()));
  ...
  void testGeneratePickerTasks() throws Exception {
    mockMvc.perform(
            get("/api/generate-picker-tasks/{date}/{numDays}/{numOfTasksPerDay}", "2023-10-01", 5, 10))
        .andExpect(status().isOk());
    verify(pickerTaskGenerator, times(1)).generatePickerTasks(any(), eq(5), eq(10), any(),
        eq(false));
  }
  ...
  void testRunWorldSimulation() throws Exception {
    mockMvc.perform(get("/api/run-world-simulation"))
        .andExpect(status().isOk());
    verify(worldSimulation, times(1)).runWorldSimulation(2, LocalDate.now());
  }
