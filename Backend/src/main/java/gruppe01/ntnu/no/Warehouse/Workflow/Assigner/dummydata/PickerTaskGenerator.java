...
public class PickerTaskGenerator {
  ...
  public List<PickerTask> generatePickerTasks(LocalDate startDate, int numDays, int numTasksPerDay,
                                              ...
                                              boolean testData) throws IOException {
      ...
      for (int i = 0; i < numDays; i++) {
        ...
        for (int j = 0; j < numTasksPerDay; j++) {
          PickerTask pickerTask =
              createPickerTask(zone, currentDate, valueList, random, testData, dueHours,
                  dueMinutes);
  ...
  private Map<List<Double>, List<List<Double>>> getMcValuesForZone(Zone zone,
                                                                   List<Map<List<Double>, List<List<Double>>>> mcValuesList) {
  ...
  private PickerTask createPickerTask(Zone zone, LocalDate date, List<List<Double>> valueList,
                                      Random random,
